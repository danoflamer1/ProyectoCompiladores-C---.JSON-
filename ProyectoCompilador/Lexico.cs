...
namespace MyCompiler
{
    internal class Lexico
    {
        ...
        Dictionary<string, string> _regexPatterns = new Dictionary<string, string>
        {
            {"Comentario", @"//.*"},
            {"ComentarioBloque", @"/\*.*?\*/"},
            {"PalabraReservada", @"\b(class|int|float|double|char|bool|return|main|void|string)\b"},
            {"Texto", "\"[^\"]*\""},
            {"Numero", @"\b\d+\b"},
            ...
            foreach (var line in lines)
            {
                ...
                while (index < line.Length)
                {
                    ...
                    if (!matchFound)
                    {
                        ...
                        Errors.Add($"Error ({lineNumber}:{index + 1}): '{simbolo}' -> Símbolo no reconocido");
                        index++;
                    }
                }
        ...
        {
            return _regexPatterns.FirstOrDefault(x => Regex.IsMatch(value, "^" + x.Value + "$")).Key ?? "Desconocido";
        }
    }
}